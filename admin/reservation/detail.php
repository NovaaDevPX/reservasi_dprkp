  ...
  <div class="max-w-5xl p-4 mx-auto main-content sm:p-6 lg:p-8">
    ...
    <div class="p-6 space-y-8 bg-white shadow rounded-2xl">
      ...
      <?php if (!empty($data['surat_pengantar'])): ?>
        <div>
          <p class="mb-3 font-semibold text-slate-800">Surat Pengantar</p>
      ...
      <div class="flex justify-end gap-4">
        <?php if ($data['status'] === 'Menunggu Admin'): ?>
          <a href="approve.php?id=<?= $data['id']; ?>"
            class="block px-4 py-2 text-left transition-all duration-300 ease-in-out border border-emerald-600 rounded-xl text-emerald-600 hover:bg-emerald-600 hover:text-white">
            Setujui
          </a>
          <a href="reject.php?id=<?= $data['id']; ?>"
            class="block px-4 py-2 text-left text-white transition-all duration-300 ease-in-out bg-red-600 border border-red-600 rounded-xl rounded-b-xl hover:bg-white hover:text-red-600 hover:border-red-600">
            Tolak
          </a>
        <?php endif; ?>
      </div>
